...
export const TargetCursor: React.FC<TargetCursorProps> = ({
  targetSelector = '.cursor-target, button, a, [role="button"], input[type="submit"]',
  spinDuration = 2,
  hideDefaultCursor = true,
  hoverDuration = 0.2,
  parallaxOn = true,
  cursorColor = '#ffffff',
  cursorColorOnTarget = '#00d2ff',
}) => {
  ...
  useEffect(() => {
    ...
    const tickerFn = () => {
      ...
      corners.forEach((corner, i) => {
        ...
        gsap.to(corner as gsap.TweenTarget, {
          x: finalX,
          y: finalY,
          duration: duration,
    ...
    const enterHandler = (e: Event) => {
      ...
      if (cursorColorOnTarget) {
        gsap.to(corners as gsap.TweenTarget, {
      ...
      const leaveHandler = () => {
        ...
        if (cursorColorOnTarget && cornersRef.current) {
          gsap.to(Array.from(cornersRef.current) as gsap.TweenTarget, {
        ...
        if (cornersRef.current) {
          const innerCorners = Array.from(cornersRef.current) as HTMLElement[];
