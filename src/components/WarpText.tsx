...
const drawLine = (
  ...
  let cursor: number;
  if (align === 'left') {
    cursor = 2; // Left-aligned with subtle padding
  } else if (align === 'right') {
    cursor = totalWidth - lineWidth - 2;
  } else {
    cursor = x - lineWidth / 2; // Centered
  }
...
const buildTextCanvas = ({ container, width, height, dpr, props }: BuildCanvasArgs) => {
  ...
  if (!Number.isFinite(lineHeight)) {
    lineHeight = fontSizePx * (typeof props.lineHeight === 'number' ? props.lineHeight : 1.1);
  }
  probe.remove();
  ...
  lines.forEach((line, index) => 
    drawLine(ctx, line, width / 2, startY + index * lineHeight, letterSpacing, textAlign, width)
  );
