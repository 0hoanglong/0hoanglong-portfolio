...
export const GlowCursor: React.FC<GlowCursorProps> = ({
  ...
  useEffect(() => {
    ...
    // Check if touch device only and disable if needed or allow desktop
    ...
    try {
      ...
      console.warn('GlowCursor WebGL could not be initialized:', e);
      return undefined;
    }
    ...
    const resize = () => {
      ...
      renderer.dpr = Math.min(window.devicePixelRatio || 1, propsRef.current.maxDevicePixelRatio);
    ...
    const updatePointer = (event: MouseEvent | PointerEvent) => {
      const dpr = Math.min(window.devicePixelRatio || 1, propsRef.current.maxDevicePixelRatio);
      const x = event.clientX * dpr;
      const y = (window.innerHeight - event.clientY) * dpr;
